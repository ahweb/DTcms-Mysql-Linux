...
namespace DTcms.Web.Plugin.submits.admin
{
    public partial class category_list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("submits_category_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("submits_category_page_size", _pagesize.ToString(), 14400);
                }
            }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(DTEnums.ActionEnum.Edit.ToString(), "保存表单分类排序"); //记录日志
            JscriptMsg("保存排序成功！", Utils.CombUrlTxt("category_list.aspx", "keywords={0}", this.keywords));
        }

        //批量删除
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("site_submits_category", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.submits_category bll = new BLL.submits_category();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(DTEnums.ActionEnum.Delete.ToString(), "删除表单分类成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("category_list.aspx", "keywords={0}", this.keywords));
        }
    }
}