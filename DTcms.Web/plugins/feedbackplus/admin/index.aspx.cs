...
namespace DTcms.Web.Plugin.FeedbackPlus.admin
{
    public partial class index : DTcms.Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("feedbackplus_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("feedbackplus_page_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("plugin_feedbackplus", DTEnums.ActionEnum.Audit.ToString()); //检查权限
            BLL.feedbackplus bll = new BLL.feedbackplus();
        ...
        {
            ChkAdminLevel("plugin_feedbackplus", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.feedbackplus bll = new BLL.feedbackplus();
            for (int i = 0; i < rptList.Items.Count; i++)
