...
namespace DTcms.Web.Plugin.submits.BLL
{
    ...
    public partial class submits_field
    {
        ...
        private readonly DAL.submits_field dal;

        public submits_field()
        {
            dal = new DAL.submits_field(siteConfig.sysdatabaseprefix);
        }
        #region  Method
        /// <summary>
        /// 是否存在该记录
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.submits_field> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.submits_field> DataTableToList(DataTable dt)
        {
            List<Model.submits_field> modelList = new List<Model.submits_field>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Model.submits_field model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Model.submits_field();
